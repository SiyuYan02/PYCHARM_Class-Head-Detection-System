...
# 设置的工作路径决定了后续所有相对路径是否有意义
os.chdir(os.path.dirname(os.path.abspath(__file__)))
# IMG_PATH = 'E:\\lwy\\Dataset\\boats\\Image'



GT_PATH = os.path.join('input', 'ground-truth')
DR_PATH = os.path.join('input', 'detection-results')
...
def log_average_miss_rate(prec, rec, num_images):
    """
        ...
        input:
                prec | precision = TP / (TP + FP)，标量
                rec  | recall = TP / (TP + FN)，标量

        output:
                lamr | log-average miss rate
                mr | miss rate = FN / (TP + FN) = 1 - rec
                fppi | false positives per image = FP / num_images = FP
    ...
    ref = np.logspace(-2.0, 0.0, num=9)     # tmp = np.linspace(-2.0, 0.0, num=9)，这里包括0.0（endpoint=True参数控制，False不包括）
                                            # ref = power(10, tmp)
