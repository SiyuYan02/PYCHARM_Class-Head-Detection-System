...
def BCELoss(pred,target):
    ...
    # alpha = 0.25
    # gamma=2
    # output = -target * (alpha * (torch.pow((1 - pred), gamma))) * torch.log(pred) \
    #          - (1.0 - target) * ((1 - alpha) * (torch.pow(pred, gamma))) * torch.log(1.0 - pred)

    return output


class YOLOLoss(nn.Module):
    def __init__(self, anchors, img_size, cuda=True):
    ...
    def forward(self, input, targets=None):
        ...
        # 找到哪些先验框内部包含物体
        mask, noobj_mask, t_box, tconf, box_loss_scale_x, box_loss_scale_y = self.get_target(targets, scaled_anchors,in_w, in_h,self.ignore_threshold)
        ...
        loss = loss_conf * self.lambda_conf + loss_loc * self.lambda_loc
        return loss, loss_conf.item(), loss_loc.item()
